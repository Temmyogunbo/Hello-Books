  ...
  */
  ...
  _createClass(SignUp, [{
    ...
    value: function onSubmit(event) {
      ...
      this.props.signup({ fullName: fullName, userName: userName, email: email, password: password }).then(function () {
        _this2.setState({ errors: _this2.props.error.message });
        console.log('ttttttt', _this2.props.error, Object.keys(_this2.props.error).length);
        if (Object(_this2.props.error).length === 0) {
          _this2.props.history.replace('/');
          _toastr2.default.success('You have successfully signed up');
        } else {
          _this2.props.history.push('/signup');
          _toastr2.default.error(_this2.props.error.error.message);
        }
      });
    }
  }, {
